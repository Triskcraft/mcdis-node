...
const token = process.env.DISCORD_TOKEN ?? config.discord.tokenEnv

if (!token) {
	throw new Error(
		`Missing Discord token. Set DISCORD_TOKEN in the environment, .env file, or discord.tokenEnv in the config.`,
	)
}
